...
export default function PanelClient() {
  ...
  const publishToFacebook = async () => {
    ...
    try {
      ...
      if (d.success) {
        setToast({ message: language === 'en' ? 'Campaign published to Facebook! (Status: PAUSED)' : '¡Campaña publicada en Facebook! (Estado: PAUSADA)', type: 'success' });
        loadFbCampaigns();
  ...
  // Force redirect to billing when plan is expired (admins can still access admin tab)
  React.useEffect(() => {
    if (isPlanExpired && activeTab !== 'billing' && !(tenantData?.isAdmin && activeTab === 'admin')) {
      setActiveTab('billing');
    }
  }, [isPlanExpired, activeTab, tenantData?.isAdmin]);

  // Guarded setActiveTab -- blocks navigation when plan is expired (admin tab exempt for admins)
  const safeSetActiveTab = (tab: typeof activeTab) => {
    if (isPlanExpired && tab !== 'billing' && !(tenantData?.isAdmin && tab === 'admin')) return;
    setActiveTab(tab);
  };
  ...
  const [adminTab, setAdminTab] = useState<'overview' | 'tenants' | 'announcements' | 'templates' | 'ai_engine'>('overview');
  ...
  return (
    ...
    <div className={`min-h-screen ${language === 'es' ? 'lang-es' : 'lang-en'} bg-crm-surface text-on-surface overflow-x-hidden font-inter`}>
      ...
      <aside className="fixed left-0 top-0 h-screen w-64 border-r border-transparent bg-[#f3f4f5] flex flex-col py-6 px-4 gap-8 shadow-[4px_0_24px_rgba(0,0,0,0.02)] z-50">
        ...
        {/* Expired Plan Sidebar Warning */}
        {isPlanExpired && (
          <div
            className="bg-gradient-to-r from-red-500 to-red-600 rounded-xl px-3 py-2.5 flex items-center gap-2 cursor-pointer hover:from-red-600 hover:to-red-700 transition-all shadow-lg shadow-red-500/20"
            onClick={() => setActiveTab('billing')}
          >
            <span className="material-symbols-outlined text-white text-lg animate-pulse">warning</span>
            <div className="flex-1 min-w-0">
              <p className="text-[10px] font-black text-white uppercase tracking-wide">{language === 'en' ? 'Plan Expired' : 'Plan Expirado'}</p>
              <p className="text-[9px] text-white/70 truncate">{language === 'en' ? 'Click to renew' : 'Click para renovar'}</p>
            </div>
        ...
        <nav className="flex-1 flex flex-col gap-1">
          ...
          {tenantData?.isAdmin && (
            <>
              ...
              <button onClick={() => safeSetActiveTab('admin')} className={`flex w-full items-center gap-3 px-4 py-3 ${activeTab === 'admin' ? 'bg-gradient-to-r from-amber-50 to-orange-50 text-orange-700 rounded-lg shadow-sm font-bold scale-[0.98]' : 'text-orange-500/70 hover:text-orange-600 font-medium'} transition-all duration-300`}>
      ...
      <main className="ml-64 pt-24 pb-12 px-10 relative overflow-y-auto h-screen">
        {/* ===== FULL LOCKOUT SCREEN — shown when plan is expired and user is NOT on billing ===== */}
        {isPlanExpired && activeTab !== 'billing' && !(tenantData?.isAdmin && activeTab === 'admin') && (
          <motion.div
            key="lockout-overlay"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            transition={{ duration: 0.4 }}
            className="fixed inset-0 ml-64 pt-16 z-30 flex items-center justify-center"
            style={{ background: 'linear-gradient(135deg, rgba(15,23,42,0.92) 0%, rgba(30,41,59,0.96) 50%, rgba(15,23,42,0.98) 100%)', backdropFilter: 'blur(12px)' }}
          >
            <div className="max-w-lg w-full mx-4 text-center">
              {/* Animated Lock Icon */}
              <div className="relative mx-auto mb-8 w-28 h-28">
                <div className="absolute inset-0 bg-red-500/20 rounded-full animate-ping" style={{ animationDuration: '2s' }}></div>
                <div className="absolute inset-2 bg-red-500/10 rounded-full animate-ping" style={{ animationDuration: '2.5s', animationDelay: '0.3s' }}></div>
                <div className="relative w-28 h-28 bg-gradient-to-br from-red-500 to-red-700 rounded-full flex items-center justify-center shadow-2xl shadow-red-500/30">
              ...
              <h2 className="text-3xl font-black text-white mb-3">
                {language === 'en' ? 'Your Plan Has Expired' : 'Tu Plan Ha Expirado'}
              </h2>
              <p className="text-slate-300 text-sm mb-8 max-w-md mx-auto leading-relaxed">
                {language === 'en'
                  ? 'Your subscription has ended. All features are locked until you renew or upgrade your plan.'
                  : 'Tu suscripción ha finalizado. Todas las funciones están bloqueadas hasta que renueves o actualices tu plan.'}
              </p>

              {/* Locked Features Grid */}
              <div className="grid grid-cols-2 gap-3 mb-8 max-w-sm mx-auto">
                {[
                  { icon: 'dashboard', label: language === 'en' ? 'Dashboard' : 'Panel' },
                  { icon: 'group', label: language === 'en' ? 'CRM & Users' : 'CRM y Usuarios' },
                  { icon: 'smart_toy', label: language === 'en' ? 'AI Playground' : 'Playground IA' },
                  { icon: 'campaign', label: language === 'en' ? 'Ad Campaigns' : 'Pautas' },
                  { icon: 'monitoring', label: language === 'en' ? 'Analytics' : 'Análisis' },
                  { icon: 'settings', label: language === 'en' ? 'Settings' : 'Configuraciones' },
                ].map((feat) => (
                  <div key={feat.icon} className="flex items-center gap-2.5 bg-white/5 border border-white/10 rounded-xl px-3 py-2.5">
              ...
              <button
                onClick={() => setActiveTab('billing')}
                className="group relative inline-flex items-center gap-3 bg-gradient-to-r from-emerald-500 to-emerald-600 text-white font-bold text-sm px-10 py-4 rounded-2xl shadow-2xl shadow-emerald-500/30 hover:shadow-emerald-500/50 hover:scale-[1.03] active:scale-[0.98] transition-all duration-300"
              >
                <span className="material-symbols-outlined text-xl">credit_card</span>
                {language === 'en' ? 'Renew My Plan Now' : 'Renovar Mi Plan Ahora'}
                <span className="material-symbols-outlined text-lg group-hover:translate-x-1 transition-transform">arrow_forward</span>
              </button>

              {/* Subtle info */}
              <p className="text-slate-500 text-[10px] mt-6 uppercase tracking-widest font-bold">
                {language === 'en' ? 'Choose a plan to restore full access' : 'Elige un plan para restaurar el acceso completo'}
              </p>
            </div>
          </motion.div>
        )}
                ...
                <section className="bg-white rounded-xl border border-[#c1c6d6] p-6" style={{ boxShadow: '0px 4px 12px rgba(0,0,0,0.05)' }}>
                  ...
                  <div className="mb-5">
                    <label className="block text-[12px] font-semibold text-[#414754] mb-2 uppercase tracking-widest flex items-center gap-2">
                      ...
                      {language === 'en' ? 'Product Image' : 'Imagen del Producto'}
                    </label>
                    ...
                    {productImagePreview ? (
                      <div className="space-y-3">
                        <div className="relative h-44 rounded-xl overflow-hidden border border-[#c1c6d6] group bg-[#f8fafc] flex items-center justify-center">
                          ...
                          <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-all flex items-center justify-center gap-2">
                            <button onClick={() => productFileRef.current?.click()} className="px-3 py-1.5 bg-white text-[#0b1c30] font-semibold text-[10px] rounded-lg shadow-lg hover:bg-slate-100 transition-all">
                              {language === 'en' ? 'Replace' : 'Reemplazar'}
                            </button>
                            <button onClick={() => { setProductImage(null); setProductImagePreview(null); }} className="px-3 py-1.5 bg-red-500 text-white font-semibold text-[10px] rounded-lg shadow-lg hover:bg-red-600 transition-all">
                      ...
                      <div onClick={() => productFileRef.current?.click()} className="border-2 border-dashed border-[#c1c6d6] rounded-xl bg-gradient-to-br from-violet-50/50 to-purple-50/50 p-6 flex flex-col items-center justify-center cursor-pointer hover:border-violet-400 hover:bg-violet-50 transition-all h-44">
                        ...
                        <p className="text-[10px] text-[#414754] mt-1">{language === 'en' ? 'The image will be used in your ad creative' : 'La imagen será usada en tu creativo publicitario'}</p>
                      </div>
                    )}
                  </div>
                      ...
                      {[...CREATIVE_TEMPLATES, ...dbTemplates].filter(tpl => 
                          ...
                          return (
                            <div
                              ...
                              onClick={() => {
                                ...
                                console.log('🎯 [Ecom Magic] Plantilla dinámica seleccionada:', tpl.name);
                              }}
                        ...
                        return (
                          <div
                            ...
                            onClick={() => {
                              setSelectedTemplate(tpl);
                              console.log('🎯 [Ecom Magic] Plantilla seleccionada:', tpl.name);
                            }}
                          ...
                          >
                            ...
                            <div className="flex flex-wrap gap-1 mt-2.5 pt-1.5 border-t border-slate-100">
                              ...
                              {tpl.layout.hasTestimonial && <span className="bg-emerald-100 text-emerald-800 text-[8px] font-bold uppercase px-1 rounded">Reseña</span>}
                ...
                <div className="bg-white rounded-xl border border-[#c1c6d6] p-6 sticky top-24" style={{ boxShadow: '0px 4px 12px rgba(0,0,0,0.05)' }}>
                  <div className="flex items-center justify-between mb-6">
                    ...
                    <div className="flex items-center gap-3">
                      {/* Cost Saver Toggle */}
                      <div className="flex items-center gap-2">
                        <span className={`text-[9px] font-bold uppercase tracking-wider ${costSaverEnabled ? 'text-amber-600' : 'text-emerald-600'}`}>
                          {costSaverEnabled ? '💰 Económico' : '✨ Premium'}
                        </span>
                        <button
                          type="button"
                          onClick={() => setCostSaverEnabled(!costSaverEnabled)}
                          className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors duration-200 focus:outline-none ${
                            costSaverEnabled ? 'bg-amber-400' : 'bg-emerald-500'
                          }`}
                        >
                          <span
                            className={`inline-block h-3.5 w-3.5 transform rounded-full bg-white shadow-sm transition-transform duration-200 ${
                              costSaverEnabled ? 'translate-x-1' : 'translate-x-[18px]'
                            }`}
                          />
                        </button>
                      </div>
                    ...
                    <div className="w-full h-full bg-white rounded-[28px] overflow-hidden flex flex-col relative">
                      ...
                      <div className="flex-1 overflow-y-auto bg-[#F0F2F5]" style={{ scrollbarWidth: 'none' }}>
                        <div className="bg-white mt-2 pb-2">
                          ...
                          {/* Ad Image */}
                          <div 
                            className="w-full bg-black relative overflow-hidden transition-all duration-300 flex items-center justify-center"
                          ...
                          >
                            {isGeneratingBanner ? (
                              <div className="absolute inset-0 flex flex-col items-center justify-center bg-gradient-to-br from-[#0058bc] to-[#1877F2]">
                                ...
                                <p className="text-white/70 text-[10px] mt-1">GPT Image AdGenius Engine</p>
                              </div>
                            ) : generatedBanner ? (
                              <div className="absolute inset-0 w-full h-full select-none overflow-hidden bg-black flex items-center justify-center">
                                {/* Completed AI Rendered Image */}
                                <img
                                  src={generatedBanner}
                                  className="w-full h-full object-contain select-none pointer-events-none"
                                  alt="Creative AI Generated Banner"
                                />
                              </div>
                            ) : (
                              <div className="absolute inset-0 flex items-center justify-center bg-slate-50">
                                <div className="bg-white/95 backdrop-blur-sm p-5 rounded-2xl shadow-md border border-slate-100 text-center max-w-[240px]">
                                  ...
                                  <p className="text-[12px] font-bold text-slate-800 leading-tight">
                                    {language === 'en' ? 'Ecom Studio Composer' : 'Composición Ecom Studio'}
                                  </p>
                                  <p className="text-[10px] text-[#414754] mt-1 mb-3.5 leading-normal">
                                    {productImagePreview 
                                      ? (language === 'en' ? 'Product uploaded & cutout ready. Compose studio banner!' : '¡Producto cargado y recorte listo! Genera tu banner de estudio.')
                  ...
                  {generatedBanner && (
                    <div className="mt-4 space-y-3">
                      <div className="flex gap-2 justify-center">
                        ...
                        <button onClick={() => generateBannerImage(campaignResult)} className="px-4 py-2 border border-amber-500 text-amber-600 text-[11px] font-semibold rounded-lg hover:bg-amber-50 transition-colors flex items-center gap-1.5">
                          <span className="material-symbols-outlined text-sm">refresh</span> {language === 'en' ? 'Retry Manually' : 'Reintentar Manualmente'}
                        </button>
                      </div>
                      ...
                      {debugTimestamp && (
                        <div className="mt-4">
                          <button
                            ...
                            className="w-full flex items-center justify-between px-4 py-2.5 bg-amber-50 border border-amber-300 rounded-lg text-amber-800 text-xs font-bold hover:bg-amber-100 transition-colors"
                          >
                            <span className="flex items-center gap-2">
                              <span className="material-symbols-outlined text-sm">bug_report</span>
                              FLUX Pipeline Debug — {new Date(debugTimestamp).toLocaleTimeString()}
                            </span>
                            <span className="material-symbols-outlined text-sm">{showDebugPanel ? 'expand_less' : 'expand_more'}</span>
                          </button>
                          {showDebugPanel && (
                            <div className="mt-2 bg-slate-900 rounded-xl p-4 border border-slate-700">
                              <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                                {[
                                  { file: 'raw_template_debug.png', label: '1. Raw Template', desc: 'Plantilla original', color: 'text-slate-400' },
                                  { file: 'cleaned_template_debug.png', label: '2. Cleaned', desc: 'Sin textos viejos', color: 'text-blue-400' },
                                  { file: 'product_cutout_debug.png', label: '3. Product Cutout', desc: 'Producto subido', color: 'text-green-400' },
                                  { file: 'product_cutout_fixed_debug.png', label: '3b. Alpha Fixed', desc: 'Producto post-alpha fix', color: 'text-lime-400' },
                                  { file: 'slot_position_diagnostic.png', label: '3c. SLOT DIAGNOSTIC', desc: '🟥 Rectángulo rojo = posición calculada', color: 'text-red-400' },
                                  { file: 'pre_composite_debug.png', label: '4. PRE-COMPOSITE', desc: '¿Tiene tu producto?', color: 'text-yellow-300' },
                                  { file: 'mask_debug.png', label: '5. Mask', desc: 'Integration ring', color: 'text-purple-400' },
                                  { file: 'final_flux_debug.png', label: '6. Final FLUX', desc: 'Resultado final', color: 'text-emerald-400' },
                                ].map((img) => (
                                  <div key={img.file} className="bg-slate-800 rounded-lg overflow-hidden border border-slate-600">
                                    <div className="relative w-full" style={{ paddingTop: '150%' }}>
                                      <img
                                        src={`/debug/${img.file}?t=${debugTimestamp}`}
                                        alt={img.label}
                                    ...
                                    <div className="p-2">
                                      <p className={`text-[10px] font-bold ${img.color}`}>{img.label}</p>
                                      <p className="text-[9px] text-slate-500">{img.desc}</p>
                                    </div>
                                  </div>
                                ))}
                              </div>
                              <div className="mt-3 p-3 bg-yellow-900/30 border border-yellow-700/50 rounded-lg">
                                <p className="text-yellow-300 text-[10px] font-bold mb-1">🔍 DIAGNÓSTICO RÁPIDO:</p>
                                <ul className="text-[9px] text-yellow-200/80 space-y-0.5">
                                  <li>• Si #4 PRE-COMPOSITE tiene el frasco original → producto NO se está pegando</li>
                                  <li>• Si #4 tiene tu producto pero #6 FINAL vuelve al frasco → FLUX sobrescribe</li>
                                  <li>• Si #3 CUTOUT tiene fondo negro → recorte sin transparencia real</li>
                                  <li>• Si #5 MASK es todo blanco → máscara no protege el producto</li>
                                </ul>
                              </div>
                            </div>
                          )}
