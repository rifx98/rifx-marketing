  ...
  useEffect(() => {
    ...
    class CarLights {
      ...
      init() {
        ...
        // Three's runtime copy supports BufferGeometry, while its declaration
        // narrows the argument to the concrete subclass through `this`.
    ...
    const resolvedOptions = {
      ...options,
      distortion: distortions[options.distortion as keyof typeof distortions] || distortions.turbulentDistortion
    };

    const myApp = new App(container, resolvedOptions);
    appRef.current = myApp;
    myApp.loadAssets().then(() => {
      // loadAssets() es async; si el efecto se limpió (unmount o doble
      // invocación de StrictMode) mientras cargaba, esta instancia ya fue
      // dispose()ada (renderer con forceContextLoss) y llamar a init() sobre
      // ella revienta con "Cannot read properties of null (reading 'alpha')".
    ...
    // effectOptions se pasa como objeto inline desde los consumidores (nueva
    // identidad en cada render), así que si estuviera en las deps, esta escena
    // WebGL completa se destruiría y recrearía en cada re-render del padre,
    // agotando los contextos WebGL del navegador y provocando crashes tipo
    // "Cannot read properties of null (reading 'alpha')" en addPass. Se
