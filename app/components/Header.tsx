...
export default function Header() {
  ...
  return (
    <>
      ...
      {/* Overlay */}
      <div
        className={`fixed inset-0 bg-black/60 backdrop-blur-sm z-[200] md:hidden transition-opacity duration-300 ${mobileOpen ? 'opacity-100 visible' : 'opacity-0 invisible pointer-events-none'}`}
        onClick={() => setMobileOpen(false)}
      />

      {/* Slide-in Panel */}
      <div 
        className={`fixed top-0 right-0 h-full w-[85%] max-w-[320px] z-[201] md:hidden transition-transform duration-300 ease-out flex flex-col ${mobileOpen ? 'translate-x-0' : 'translate-x-full'}`}
        style={{ backgroundColor: 'rgba(11, 18, 41, 0.98)', backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)' }}
      >
        ...
        <div className="px-6 pt-6 pb-4 border-b border-white/10 flex items-center justify-between">
          ...
          <button onClick={() => setMobileOpen(false)} className="w-8 h-8 flex items-center justify-center text-white/60 hover:text-white transition-colors">
        ...
        <div className="flex-1 overflow-y-auto py-6 px-4 space-y-1">
          <Link 
            href="/" 
            onClick={() => setMobileOpen(false)} 
            className={`flex items-center gap-3 px-4 py-3.5 rounded-xl text-sm font-semibold uppercase tracking-wider transition-all ${pathname === '/' ? 'bg-rocket-orange/15 text-rocket-orange' : 'text-slate-200 hover:bg-white/5'}`}
          >
            🏠 Inicio
          </Link>
          ...
          <button
            ...
            className={`w-full flex items-center justify-between px-4 py-3.5 rounded-xl text-sm font-semibold uppercase tracking-wider transition-all ${pathname?.startsWith('/servicios') ? 'bg-rocket-orange/15 text-rocket-orange' : 'text-slate-200 hover:bg-white/5'}`}
          >
            <span>🚀 Servicios</span>
            <svg 
          ...
          {/* Services Sub-links */}
          <div className={`overflow-hidden transition-all duration-300 ease-in-out ${servicesOpen ? 'max-h-[300px] opacity-100' : 'max-h-0 opacity-0'}`}>
            <div className="ml-4 pl-4 border-l-2 border-rocket-orange/30 space-y-1 py-2">
              <Link href="/servicios/anuncios-de-alta-velocidad" onClick={() => setMobileOpen(false)} className="block px-3 py-2.5 rounded-lg text-slate-300 hover:text-white hover:bg-white/5 transition-all text-sm">
              ...
              <Link href="/servicios/ecommerce-interestelar" onClick={() => setMobileOpen(false)} className="block px-3 py-2.5 rounded-lg text-slate-300 hover:text-white hover:bg-white/5 transition-all text-sm">
          ...
          <Link 
            href="/contacto" 
            onClick={() => setMobileOpen(false)} 
            className={`flex items-center gap-3 px-4 py-3.5 rounded-xl text-sm font-semibold uppercase tracking-wider transition-all ${pathname === '/contacto' ? 'bg-rocket-orange/15 text-rocket-orange' : 'text-slate-200 hover:bg-white/5'}`}
          >
            ✉️ Contacto
          </Link>
        </div>
        ...
        <div className="px-6 py-6 border-t border-white/10">
          <Link 
            href="/contacto" 
            onClick={() => setMobileOpen(false)} 
            className="block w-full bg-rocket-orange text-white text-center px-6 py-3.5 rounded-xl font-bold shadow-[0_0_20px_rgba(242,113,33,0.4)] transition-all text-sm uppercase tracking-wider"
          >
            🚀 ¡Empezar Ahora!
          </Link>
        </div>
